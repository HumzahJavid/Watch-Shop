...
function loadBasket(){
    ...
    for(var i=0; i<basketArray.length; ++i){
        prodIDs.push({id: basketArray[i].id, name: basketArray[i].name, count: 1, productID: basketArray[i].productID, price: basketArray[i].price});//Add to product array
    }
...
function addToBasket(prodID, prodName, productID, price){
    //Get basket from local storage or create one if it does not exist
    var basketArray;
    if(sessionStorage.basket === undefined || sessionStorage.basket === ""){
        basketArray = [];
    ...
    basketArray.push({id: prodID, name: prodName, productID: productID, price: price});
